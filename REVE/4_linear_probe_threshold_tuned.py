...
def tune_threshold(y_true, y_prob, thresholds=None):
    ...
    results_df = pd.DataFrame(rows).sort_values("threshold").reset_index(drop=True)
    return best_thr, results_df
...
def main():
    ...
    print("\nTEST RESULTS (default threshold)")
    for k, v in test_metrics.items():
        print(f"{k}: {v:.4f}")

    val_y, val_prob = collect_probs(model, val_loader)
    best_thr, thr_table = tune_threshold(val_y, val_prob)

    print("\nBest threshold on validation (by F1):", f"{best_thr:.2f}")
    print("\nTop 10 validation thresholds by F1")
    print(thr_table.sort_values("f1", ascending=False).head(10).to_string(index=False))

    test_y, test_prob = collect_probs(model, test_loader)
    test_thr_metrics = evaluate_at_threshold(test_y, test_prob, best_thr)

    print("\nTEST RESULTS WITH TUNED THRESHOLD")
    for k, v in test_thr_metrics.items():
