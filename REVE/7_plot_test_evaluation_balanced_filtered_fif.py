...
OUT_DIR = ROOT / f"plots_{TEST_SUBJECT}_mono_balanced_filtered_fif"
...
def save_confusion_matrix(df):
    ...
    ax.set_title(
        f"Confusion Matrix | {TEST_SUBJECT} | threshold={THRESHOLD:.2f}"
    )
...
def save_probability_histogram(df):
    ...
    plt.hist(
        df.loc[df["y_true"] == 0, "prob_seizure"],
        bins=50,
        alpha=0.6,
        label="True non-seizure",
    )

    plt.hist(
        df.loc[df["y_true"] == 1, "prob_seizure"],
        bins=50,
        alpha=0.6,
        label="True seizure",
    )
...
def save_threshold_sweep(df):
    ...
    thr_df.to_csv(
        OUT_DIR / "threshold_sweep_metrics.csv",
        index=False,
    )
...
def save_probability_timelines(df):
    ...
    for edf_name, g in df.groupby("edf"):
        ...
        plt.plot(
            ...
            y_prob,
            linewidth=1.5,
            label="Predicted seizure probability",
        )
        ...
        seizure_regions = find_contiguous_regions(
            x,
            x_end,
            y_true,
        )
